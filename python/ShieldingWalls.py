...
def make_placement(geom, position, name,volume):
...
class ShieldingWallsBuilder(gegede.builder.Builder):
    ...
    def construct(self, geom):
        ...
        BlockWidth = fSpacing - fIFlangeWaist - Q('0.00001cm') - Q('0.0005cm')
        BlockHeight = fIPortSpacing - Q('0.0005cm')
        BlockHeightTop = BlockHeight * 0.63
        BlockHeightBottom = BlockHeight * 0.15

        fBlockThickness = Q('0.004cm')
        ...
        # fcRotation = geom.structure.Rotation('fc', x= "0deg", y= "0deg",z= "90deg")
        # fc2Rotation = geom.structure.Rotation('fc2',x= "0deg", y= "90deg",z= "0deg")
        # fc3Rotation = geom.structure.Rotation('fc3', x= "90deg", y= "0deg",z= "90deg")
        ...
        for ii in range(19):
            for jj in range(5):
                ...
                if(jj ==3):
                    y = yTopHole + BlockHeight/2+ BlockHeightTop/2 + Q('0.0005cm')
                ...
                if(jj==1):
                    y = y1HoleUp
                if(jj==4):
                    y= (yBotHole)-(BlockHeight/2) - (0.05*BlockHeightBottom)- (Q('0.0005cm'))
                    shield = ShieldBlockBotLog  


                #pdb.set_trace()
                shieldLeftPosition = make_position(geom , f'ShieldLeftPosition{ii}{jj}',-st,y,zpl)
                shieldLeft = make_placement(geom, shieldLeftPosition,f'ShieldLeftPlacement{ii}{jj}', shield)
                self.PlacementList.append(shieldLeft)

                shieldRightPosition = make_position(geom , f'ShieldRightPosition{ii}{jj}',st,y,-zpl)
                shieldRight = make_placement(geom, shieldRightPosition,f'ShielRightPlacement{ii}{jj}',  shield)
                self.PlacementList.append(shieldRight)

                shieldLeftPosition = make_position(geom , f'ShieldLeft2Position{ii}{jj}',-st,y,zpl)
                shieldLeft = make_placement(geom, shieldLeftPosition,f'ShieldLeft2Placement{ii}{jj}',  shield)
                self.PlacementList.append(shieldLeft)

                shieldRightPosition = make_position(geom , f'ShieldRight2Position{ii}{jj}',st,y,zpl)
                shieldRight = make_placement(geom, shieldRightPosition,f'ShielRight2Placement{ii}{jj}',  shield)
                self.PlacementList.append(shieldRight)
        ...
        for ii in range(5):
            for jj in range(5):
                ...
                if(jj==0):
                    y =yTopHole
                if(jj==4):
                    y = yBotHole -BlockHeight/2 - 0.5*BlockHeightBottom - Q('0.0005cm')

                ShieldBackPositon = make_position(geom, f'ShieldBackPosition{ii}{jj}', -xpl, y, -zpl)
                ShielBackPlacement = make_placement(geom,ShieldBackPositon,f'ShieldBackPlacement{ii}{jj}', shield)
                self.PlacementList.append(ShielBackPlacement)

                ShieldBackPositon = make_position(geom, f'ShieldBack2Position{ii}{jj}', xpl, y, -zpl)
                ShielBackPlacement = make_placement(geom,ShieldBackPositon,f'ShieldBack2Placement{ii}{jj}', shield)
                self.PlacementList.append(ShielBackPlacement)

                ShieldFrontPosition = make_position(geom, f'ShieldFrontposition{ii}{jj}', -xpl,y,zpl)
                ShieldFrontPlacement = make_placement(geom,ShieldFrontPosition,f'ShieldFrontPlacement{ii}{jj}',shield)
                ...
                ShieldFrontPlacement = make_placement(geom,ShieldFrontPosition,f'ShieldFront2Placement{ii}{jj}',shield)
                self.PlacementList.append(ShieldFrontPlacement)
            xpl+=zbsp
